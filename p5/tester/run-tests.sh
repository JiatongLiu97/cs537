...
run_test () {
    ...
    # (Mod) timeout support
    eval timeout 30 $(cat $testfile) > $outdir/$testnum.out 2> $outdir/$testnum.err
    echo $? > $outdir/$testnum.rc

    if (( $testnum == 1 )); then
        # (Mod) Valgrind test
        eval valgrind --error-exitcode=26 timeout 1 $(cat $testfile) > $outdir/$testnum.valgrind.out 2> $outdir/$testnum.valgrind.err
